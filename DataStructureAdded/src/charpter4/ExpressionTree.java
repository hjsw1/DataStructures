...
public class ExpressionTree {
	...
	public static BiTree<Character> createTreeUseInOrderExpression(String exp) {
	...
	public static BiTree<Character> createTreeUsePostOrderExpression(String exp) {
		...
		for (int i = 0; i < exps.length; i++) {
			if (isOperator(exps[i])) {
				Lchild = nodes.pop();
				Rchild = nodes.pop();
				root = new TreeNode<>(exps[i], Lchild, Rchild);
				nodes.push(root);
			} else {
				Lchild = new TreeNode<>(exps[i]);
				nodes.push(Lchild);
